...
def getVocabularies(debugOutputFileHandle):
    ...
    getSQL = "SELECT vid, name FROM vocabulary WHERE hierarchy > 0 ORDER BY vid"
...
def getTaxonomyTopLevel(debugOutputFileHandle, vocabularyID):
    ...
    getSQL = "SELECT term_data.tid, name "
    getSQL = getSQL + "FROM term_data "
    getSQL = getSQL + "LEFT JOIN term_hierarchy "
    getSQL = getSQL + "ON (term_data.tid = term_hierarchy.tid) "
    getSQL = getSQL + "WHERE vid = " + str(vocabularyID) + " "
    getSQL = getSQL + "AND term_hierarchy.parent = 0 "
    getSQL = getSQL + "ORDER BY name, weight, term_data.tid"
...
def printChildren(debugOutputFileHandle, outputFileHandle, vocabularyID, childTID, depth):
    ...
    getSQL = getSQL + "ON (term_data.tid = term_hierarchy.tid) "
    getSQL = getSQL + "WHERE vid = " + str(vocabularyID) + " "
    getSQL = getSQL + "AND term_hierarchy.parent = " + str(childTID) + " "
    getSQL = getSQL + "ORDER BY name, weight, term_data.tid"
